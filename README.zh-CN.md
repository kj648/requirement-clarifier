...
生成的确认单——白卡蓝墨、悬停展开的题号索引、吸顶进度条、「我们理解的」逐条核对：
...
- ✅ **189 条回归测试**（纯 Python 标准库 `unittest`，零第三方依赖）
- ✅ **证据核验**——每条 `> 证据: 路径:行号 | "原文片段"` 引用逐条对照源文件，断链即 FAIL
- ✅ **确认单契约校验**——依赖悬空、分支不对称、规则题带建议措辞、阻塞题缺演示数字：出包前全部拒收
- ✅ **回执机检**（七条规则）——阻塞题未答、矛盾未说明、「我们理解的」没核对、业务证伪的题、勾了「不清楚」却没给知情人、多选、模板残留，在答案被合并之前拦住
...
│   ├── check_questionnaire.py        # 回执机检（七件事）
