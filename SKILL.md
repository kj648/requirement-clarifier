...
问题清单要求：只列业务才能回答的（技术选型开发自己定）；一律选择题化（业务擅长选 A/B，不擅长从零描述）；已有答案的不问；按阻塞程度排序；**每题按 `decide: biz|dev` 标档**——biz=业务必须自己定，dev=开发已拟默认规则请业务过目；**给谁去问是开发的责任**：知情人常散在多拨人手里（提需求业务、财务、外围系统操作者等），出题前自己先确认清楚知情人是谁、再决定这份单子发给谁，**单子里不体现具体收件人**。确认单额外要求：①开头附"已确认事项核对表"，把口头共识变成有落款的凭证；②每题 ☐ 选项 + 作答区，按 decide 标"业务定"或"开发拟定 · 请过目"；③给"我不清楚"台阶，顺势索要真正知情人。
...
**落款检查：** 回收确认单核对填写人/日期；回执（确认单/聊天/邮件）归档进 `raw/`，spec 确认记录引用回执行号。缺落款提醒补。注意 provenance：业务"说过"≠"决定了"，随口畅想按【假设】入账。**阻塞级的题不许用【开发拟定】顶过去**，只能推迟开发或向拍板人升级。
