...
- **`code` 的风险是单点引用冒充整体逻辑**：一行赋值不证明它是唯一赋值点、没有别处覆盖、
  没有 flag 短路。三条全 PASS 的引用照样能让业务确认一个错误的现状——**而且是业务
  亲自过目过、进了归档回执的错误**，比没有证据更糟。尤其危险是演示数字照这个逻辑算的：逻辑读了一半，业务就照错
  数字选口径。
...
- 「都不是」兜底出口由模板自动追加，**不要写进 json**；它导出时带 `kind: "other"`

`kind` 不只是页面样式：导出回执时它原样写进机读区的 `主选kind`，机检的「勾了『不清楚』
却没给知情人」这条据此判定。**该标的没标 = 这条规则对那道题静默失效**（回执里没有
语义标记，机检只能退回去猜 label 的字面，英文单子基本猜不中）。`kind: "other"` 则是
阶段三判「选项集猜错了、需要下一轮」的结构信号。

## 九、单子的语言

`doc.lang: "zh" | "en"`（缺省 `zh`）只管**页面外壳**——抬头、表头、按钮、提示、回执骨架词，
字符串表在 `scripts/i18n.py`，中英各一份、HTML 与 md 同源。

**题目内容不经过这个字段**：标题、选项 label、背景、`weak`、`cost` 由你按交互语言直接
写进 json。语言写错了就重写一遍 json，不需要任何机制。

不翻译的东西：机读区的 JSON 键名、三档来源标签（【业务确认】/【开发拟定】/【假设】）。
它们是协议，像 HTTP 头——业务不读，机检和 AI 读。翻了它们等于把协议改了。

`--md` 出的静态单子末尾会附一个机读区，里面是这份单子的**锚点表**（结构词用的是哪几个
词）与题号表。机检据此把英文结构词还原成中文规范词再跑原来的判据；中文单子是恒等映射，
产物与结论逐字节不变。手改了单子的结构词而不改锚点表，机检会报「题数对不上」。
